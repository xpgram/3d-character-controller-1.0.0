...
## Camera maximum swivel speed in, I dunno, angles per second.
@export var camera_speed := 2.25
...
func _physics_process(delta: float) -> void:
	...
	# This and mouse-look above will compound, but w/e 
	var raw_cam_input := Input.get_vector('look_left', 'look_right', 'look_up', 'look_down', 0.4)
	var curved_cam_input := raw_cam_input * raw_cam_input.length()
	_camera_pivot.rotation.x += curved_cam_input.y * camera_speed * delta
	_camera_pivot.rotation.x = clamp(_camera_pivot.rotation.x, tilt_lower_limit, tilt_upper_limit)
	_camera_pivot.rotation.y -= curved_cam_input.x * camera_speed * delta
